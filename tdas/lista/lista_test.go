...
// Agrego test restantes.
func TestRecorrerIterador(t *testing.T) {
	lista := TDALista.CrearListaEnlazada[int]()
	...
	posicion := 0
	for iterador.HaySiguiente() {
		require.EqualValues(t, posicion, iterador.VerActual())
		posicion++
		iterador.Siguiente()
	}
	require.PanicsWithValue(t, "El iterador termino de iterar", func() { iterador.Siguiente() })
...
func TestInsertarPrincipioFinalIterador(t *testing.T) {
	lista := TDALista.CrearListaEnlazada[int]()
