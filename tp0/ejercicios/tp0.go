...
// Recibe un arreglo, y la posicion desde la que se quiere comenzar a sumar.
func sumaRec(vector []int, posicion_actual int) int {
	if posicion_actual == len(vector) {
		return 0
	}
	return vector[posicion_actual] + sumaRec(vector, posicion_actual+1)
}
